...
- (void)gotSources:(id)result
{
	...
	if (![[result objectForKey:@"failure"] boolValue])
    {
		...
		if ([[result objectForKey:@"result"] objectForKey:@"sources"])
		{
			NSArray* sources = [[result objectForKey:@"result"] objectForKey:@"sources"];
			items = [NSMutableArray arrayWithCapacity:[sources count]];

...
- (void)setTabs:(NSArray *)array {
	...
	for (BCTab *tab in tabs) {
		...
		UITapGestureRecognizer* recognizer = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(tabSelectedByTap:)];
...
- (void)tabSelectedByTap:(UITapGestureRecognizer *)recognizer 
{
    if (recognizer.state != UIGestureRecognizerStateRecognized) return;
    {
		[self.delegate tabBar:self didSelectTabAtIndex:
						[self.tabs indexOfObject:((BCTab*)recognizer.view)]];
    }
}

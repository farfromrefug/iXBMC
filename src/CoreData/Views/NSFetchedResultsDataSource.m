...
@synthesize sortNames = _sortNames;
...
- (id) initWithEntity:(NSEntityDescription *)entity controllerTableView:(UITableView *)controllerTableView{
	...
	if(self)
    {
		...
		self.sortBy = [class defaultSort];
		self.sortArray = [class sorts];
		self.sortNames = [class sortNames];
...
- (void) loadLocal:(BOOL)more{
	...
	[request setSortDescriptors:[ActiveSupport sortDescriptorsFromString:_sortBy]];
    if (_selectFields) {
	...
	_fetchedResultsController = [[NSFetchedResultsController alloc] initWithFetchRequest:request managedObjectContext:[NSManagedObjectContext MR_defaultContext] sectionNameKeyPath:_sectionKey cacheName:nil];
